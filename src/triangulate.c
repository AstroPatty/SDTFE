...
// This function triangulates the point list ( pts ) and constructs a
// tetrahedron list ( tetra_lst )
int *triangulate(double *particle_data, int n_particles, size_t *num_facets,
                 char *options) {
  ...
  FORALLfacets {
    if (!facet->upperdelaunay) {
      ...
      for (k = 0; k < 3; ++k) {
        ab[k] =
            particle_data[verts1[B] * 4 + k] - particle_data[verts1[A] * 4 + k];
        ac[k] =
            particle_data[verts1[C] * 4 + k] - particle_data[verts1[A] * 4 + k];
        ad[k] =
            particle_data[verts1[D] * 4 + k] - particle_data[verts1[A] * 4 + k];
      }
    ...
    if (!facet->upperdelaunay) {
      ...
      FOREACHridge_(facet->ridges) {
        ...
        if (!neighbor->upperdelaunay) {
          ...
          for (k = 0; k < 4; k++) {
            if ((verts1[k] == verts2[0]) || (verts1[k] == verts2[1]) ||
                (verts1[k] == verts2[2]) || (verts1[k] == verts2[3])) {
              comp[k] = 1;
            }
          }
...
void pre_compute_vol(double **particle_data, int n_particles, int *tetra_data,
                     int n_tetra, float p_mass) {
  double vol;
  int i;
  ...
  for (i = 0; i < n_tetra; ++i) {
    vol = (((*particle_data)[tetra_data[i * 8 + A] * 4 + 0] -
            (*particle_data)[tetra_data[i * 8 + D] * 4 + 0]) *
               (((*particle_data)[tetra_data[i * 8 + B] * 4 + 1] -
                 (*particle_data)[tetra_data[i * 8 + D] * 4 + 1]) *
                    ((*particle_data)[tetra_data[i * 8 + C] * 4 + 2] -
                     (*particle_data)[tetra_data[i * 8 + D] * 4 + 2]) -
                ((*particle_data)[tetra_data[i * 8 + B] * 4 + 2] -
                 (*particle_data)[tetra_data[i * 8 + D] * 4 + 2]) *
                    ((*particle_data)[tetra_data[i * 8 + C] * 4 + 1] -
                     (*particle_data)[tetra_data[i * 8 + D] * 4 + 1])) +
           ((*particle_data)[tetra_data[i * 8 + A] * 4 + 1] -
            (*particle_data)[tetra_data[i * 8 + D] * 4 + 1]) *
               (((*particle_data)[tetra_data[i * 8 + B] * 4 + 2] -
                 (*particle_data)[tetra_data[i * 8 + D] * 4 + 2]) *
                    ((*particle_data)[tetra_data[i * 8 + C] * 4 + 0] -
                     (*particle_data)[tetra_data[i * 8 + D] * 4 + 0]) -
                ((*particle_data)[tetra_data[i * 8 + B] * 4 + 0] -
                 (*particle_data)[tetra_data[i * 8 + D] * 4 + 0]) *
                    ((*particle_data)[tetra_data[i * 8 + C] * 4 + 2] -
                     (*particle_data)[tetra_data[i * 8 + D] * 4 + 2])) +
           ((*particle_data)[tetra_data[i * 8 + A] * 4 + 2] -
            (*particle_data)[tetra_data[i * 8 + D] * 4 + 2]) *
               (((*particle_data)[tetra_data[i * 8 + B] * 4 + 0] -
                 (*particle_data)[tetra_data[i * 8 + D] * 4 + 0]) *
                    ((*particle_data)[tetra_data[i * 8 + C] * 4 + 1] -
  ...
  for (i = 0; i < n_particles; ++i)
    (*particle_data)[i * 4 + 3] =
        p_mass * (1 + DIM) / (*particle_data)[i * 4 + 3];
}

static inline double determ3x3(double a, double b, double c, double d, double e,
...
double *gradients(int *tetra_data, int n_tetra, double *particle_data) {
  ...
  int i;
  for (i = 0; i < n_tetra; ++i) {
    D_xb = particle_data[tetra_data[i * 8 + B] * 4 + 0] -
           particle_data[tetra_data[i * 8 + A] * 4 + 0];
    D_yb = particle_data[tetra_data[i * 8 + B] * 4 + 1] -
           particle_data[tetra_data[i * 8 + A] * 4 + 1];
    D_zb = particle_data[tetra_data[i * 8 + B] * 4 + 2] -
           particle_data[tetra_data[i * 8 + A] * 4 + 2];
    D_xc = particle_data[tetra_data[i * 8 + C] * 4 + 0] -
           particle_data[tetra_data[i * 8 + A] * 4 + 0];
    D_yc = particle_data[tetra_data[i * 8 + C] * 4 + 1] -
           particle_data[tetra_data[i * 8 + A] * 4 + 1];
    D_zc = particle_data[tetra_data[i * 8 + C] * 4 + 2] -
           particle_data[tetra_data[i * 8 + A] * 4 + 2];
    D_xd = particle_data[tetra_data[i * 8 + D] * 4 + 0] -
           particle_data[tetra_data[i * 8 + A] * 4 + 0];
    D_yd = particle_data[tetra_data[i * 8 + D] * 4 + 1] -
           particle_data[tetra_data[i * 8 + A] * 4 + 1];
    D_zd = particle_data[tetra_data[i * 8 + D] * 4 + 2] -
           particle_data[tetra_data[i * 8 + A] * 4 + 2];

    D_rb = particle_data[tetra_data[i * 8 + B] * 4 + 3] -
           particle_data[tetra_data[i * 8 + A] * 4 + 3];
    D_rc = particle_data[tetra_data[i * 8 + C] * 4 + 3] -
           particle_data[tetra_data[i * 8 + A] * 4 + 3];
    D_rd = particle_data[tetra_data[i * 8 + D] * 4 + 3] -
           particle_data[tetra_data[i * 8 + A] * 4 + 3];

    double denom =
        determ3x3(D_xb, D_yb, D_zb, D_xc, D_yc, D_zc, D_xd, D_yd, D_zd);

    nabla[i * 3 + 0] =
        determ3x3(D_rb, D_yb, D_zb, D_rc, D_yc, D_zc, D_rd, D_yd, D_zd) / denom;
    nabla[i * 3 + 1] =
        determ3x3(D_xb, D_rb, D_zb, D_xc, D_rc, D_zc, D_xd, D_rd, D_zd) / denom;
    nabla[i * 3 + 2] =
        determ3x3(D_xb, D_yb, D_rb, D_xc, D_yc, D_rc, D_xd, D_yd, D_rd) / denom;
  }
  return nabla;
}

// This function determines which tetrahedra have a face on the forward hull
int find_forward_hull(int *tetra_data, int n_tetra, double *particle_data,
                      int n_particles, word_t *bitmap) {
  ...
  for (i = 0; i < n_tetra; ++i) {
    ...
    for (j = 0; j < 4; ++j) {
      ...
      if (neighbor_id == -1) {
        ...
        case ABC:
          for (k = 0; k < 3; ++k) {
            t1[k] = particle_data[tetra_data[i * 8 + B] * 4 + k] -
                    particle_data[tetra_data[i * 8 + A] * 4 + k];
            t2[k] = particle_data[tetra_data[i * 8 + C] * 4 + k] -
                    particle_data[tetra_data[i * 8 + A] * 4 + k];
          }
          break;
        case ADB:
          for (k = 0; k < 3; ++k) {
            t1[k] = particle_data[tetra_data[i * 8 + D] * 4 + k] -
                    particle_data[tetra_data[i * 8 + A] * 4 + k];
            t2[k] = particle_data[tetra_data[i * 8 + B] * 4 + k] -
                    particle_data[tetra_data[i * 8 + A] * 4 + k];
          }
          break;
        case ACD:
          for (k = 0; k < 3; ++k) {
            t1[k] = particle_data[tetra_data[i * 8 + C] * 4 + k] -
                    particle_data[tetra_data[i * 8 + A] * 4 + k];
            t2[k] = particle_data[tetra_data[i * 8 + D] * 4 + k] -
                    particle_data[tetra_data[i * 8 + A] * 4 + k];
          }
          break;
        case BDC:
          for (k = 0; k < 3; ++k) {
            t1[k] = particle_data[tetra_data[i * 8 + D] * 4 + k] -
                    particle_data[tetra_data[i * 8 + B] * 4 + k];
            t2[k] = particle_data[tetra_data[i * 8 + C] * 4 + k] -
                    particle_data[tetra_data[i * 8 + B] * 4 + k];
          }
          break;
...
int *copy_fh_triangles(int *tetra_data, int n_tetra, double *particle_data,
                       int n_particles, int n_tri, word_t *bitmap) {
  ...
  //*******************************************************************************
  // find the neighbors in the set of faces (exhaustive, but ralatively small in
  // number)
  for (i = 0; i < T_id; ++i) {
    for (j = 0; j < T_id; ++j) {
      if (i != j) {
